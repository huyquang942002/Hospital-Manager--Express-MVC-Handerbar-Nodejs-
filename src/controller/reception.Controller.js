...
const updateSlot = (nameDoctor, lengthDoctor) => {
  ...
  fs.writeFileSync("./src/models/doctor/doctor.json", JSON.stringify(doctor));
  return doctor;
};
...
const moveReception = (req, res) => {
  ...
  for (let i = 0; i < reception.length; i++) {
    ...
    for (let j = 0; j < doctor.length; j++) {
      ...
      const fileName = `./src/models/doctor/${doc.name}.json`;

      data = JSON.parse(fs.readFileSync(fileName));

      if (
        patientReception.specialist === doc.specialist &&
        data.length < doc.slot
      ) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(removeReception(arr, reception))
  );
